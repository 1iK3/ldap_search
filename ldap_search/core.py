...
from ldap_search.query import QUERIES, ATTRIBUTES
...
class LdapEnum():
    ...
    def ldap_connection(self,):
        if self.create_ldap_con():
            return True
        elif self.create_ldaps_con():
            return True
        return False
    ...
    def ldap_auth(self, user, passwd, hash, domain):
        ...
        if hash:
            try:
                self.lm, self.nt = hash.split(':')
            except:
                self.nt = hash
            self.con.login(user, '', domain, lmhash=self.lm, nthash=self.nt)
        else:
    ...
    def custom_query(self, query, attrs):
        ...
        return self.ldap_query(query, attrs, self.no_parser)
